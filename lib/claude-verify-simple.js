...
class SimpleClaudeVerifier {
    ...
    buildPrompt(context) {
        ...
        let stopInput = {};
        try {
            if (process.env.STOP_HOOK_INPUT) {
                stopInput = JSON.parse(process.env.STOP_HOOK_INPUT);
            }
        } catch (err) {
            // 忽略解析错误
        }
        ...
        if (fs.existsSync(templatePath)) {
            ...
            template = template.replace(/\{\{stopInput\}\}/g, JSON.stringify(stopInput, null, 2));
            // 支持访问 stopInput 的嵌套字段（支持多层嵌套如 stopInput.user.name）
            template = template.replace(/\{\{stopInput\.([a-zA-Z_][\w.]*)\}\}/g, (match, path) => {
                // 解析嵌套路径
                ...
                return value !== undefined && value !== null ? String(value) : `(无${path})`;
            });
        } else {
            // 使用默认模版
